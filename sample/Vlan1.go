...
func main() {
	...
	for net := 0; net <= 600; net++ {
		<-ticker.C
		c.RunCommand(cli.Command{"config", "interface vlan " + fmt.Sprintf("%v", intf), "#"})
		c.Sendln("shutdown")
		c.Expect("#")
		c.RunCommand(cli.Command{"config", "no interface vlan " + fmt.Sprintf("%v", intf), "#"})
		c.RunCommand(cli.Command{"config", "vlan database", "#"})
		c.Sendln("no vlan " + fmt.Sprintf("%v", intf))
		c.Expect("#")
		intf += 1
	}
