...
func main() {
	...
	c.RunCommand(cli.Command{"config", "router ospf", "#"})
	ticker := time.NewTicker(time.Second / 10)
	for net := 0; net <= 2000000; net++ {
		c.Sendln("redistribute static subnet")
		c.Expect("#")
		<-ticker.C
		c.Sendln("no redistribute static subnet")
		c.Expect("#")
		c.Sendln("redistribute bgp subnets")
		c.Expect("#")
		<-ticker.C
		c.Sendln("no redistribute bgp")
		c.Expect("#")
	}
